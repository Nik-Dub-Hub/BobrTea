...
class CommentController {
  ...
  static async getCommentByTeaId(req, res) {
    ...
    try {
      const comment = await CommentService.getAllByTeaId(reformatId(tea_id));
      if (!comment) {
        return res
          .status(404)
          .json(
            formatResponse(404, `Comment with tea_id ${tea_id} not found`)
          );
      }
      res.status(200).json(formatResponse(200, "success", comment));
    } catch ({ message }) {
